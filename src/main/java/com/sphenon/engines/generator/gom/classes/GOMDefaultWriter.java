...
public class GOMDefaultWriter extends Class_GOMNode {

    // Configuration ------------------------------------------------------------------

    // Construction -------------------------------------------------------------------

    public GOMDefaultWriter(CallContext context, int id) {
        super(context, null, id);
    }

    public GOMDefaultWriter(CallContext context, GOMNode parent, int id) {
    ...
    public boolean generate (CallContext context, GOMProcessor gom_processor, GOMExecutionContext gom_execution_context, GeneratorOutputHandler output_handler, Object... arguments) {
        ...
        if (local == null) {
            ...
            local.output_handler = output_handler;
            local.output_handler.openHandler(context);
            ((Class_GOMExecutionContext) gom_execution_context).setOutputHandler(context, local.output_handler);
        } else {
            local.index++;
        }
        if (local.index < this.getChildNodes(context).size()) {
            ...
            local.output_handler.closeHandler(context);
